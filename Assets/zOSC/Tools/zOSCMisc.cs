...
#pragma warning restore 649
    ...
    bool RestartLocalServer()
    {
        ...
        if (OSCHandler.Instance.Servers.ContainsKey("LocalHost"))
        {
            ...
            if (logToConsole) Debug.Log("zOSC localhost open running on port : " + OSCHandler.Instance.Servers["LocalHost"].server.LocalPort);
        }
        return true;
    }
    ...
    public static zOSC instance
    {
        get
        {
            if (_instance == null)
            {
                //                throw new System.InvalidOperationException("zOSC instance not present on scene, or its AWAKE has not been run yet");
            }
            return _instance;
        }
    }
    ...
    */
    ...
    {
        if (_instance != null)
            Debug.Log(smallBegin + "zOSC: " + s + smallEnd);
    }

    public static void LogSend(string s)
    {
        if (_instance != null)
            Debug.Log(smallBegin + "zOSC: " + sentString + s + smallEnd);
    }

    public static void LogReceived(string s)
    {
        if (_instance != null)
            Debug.Log(smallBegin + "zOSC: " + receivedString + s + smallEnd);
    }
    public static void Log(string s, GameObject g)
    {
        if (_instance != null && _instance.detailedLog)

            Debug.Log(smallBegin + "zOSC: " + s + smallEnd, g);
    }
    ...
    public static OSCRouter mainRouter
    {
        get
        {
            if (_mainRouter == null)
            {
                ...
                {
                    var osc = GameObject.FindObjectOfType<zOSC>();
                    if (osc != null)
                    {
                        Debug.Log("strainge");
                    ...
                    {
                        GameObject game = new GameObject("zOSC");
                        game.AddComponent<zOSC>();
